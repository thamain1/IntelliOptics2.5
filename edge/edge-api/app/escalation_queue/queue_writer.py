...
class QueueWriter:
    ...
    def write_escalation(self, escalation_info: EscalationInfo) -> bool:
        ...
        Grouping by priority ensures the reader's file-level sort reflects the intended order.

        Returns True if the write succeeds and False otherwise.
        """
        # ── Item 4: Escalation Queue Priority ───────────────────────────────
        # Start a new file when priority changes so all lines in a file share
        # the same priority and the reader's filename sort stays accurate.
        is_new_file = False
    ...
    # QueueReader sorts filenames lexicographically; a higher-priority file
    # gets a higher numeric prefix and a custom sort key so it is consumed
    # before lower-priority files at the same timestamp.
